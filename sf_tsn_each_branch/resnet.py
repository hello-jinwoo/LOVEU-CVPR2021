...
class ResNet(nn.Module):
    def __init__(self, in_planes, decoder_hidden, block, num_blocks, num_classes=10):
        ...
        self.conv1 = nn.Conv2d(in_planes, 64, kernel_size=3,
                               stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=1)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=1)
        self.layer4 = self._make_layer(block, decoder_hidden, num_blocks[3], stride=1)
...
def ResNet18(in_planes, decoder_hidden):
    return ResNet(in_planes, decoder_hidden, BasicBlock, [2, 2, 2, 2])
